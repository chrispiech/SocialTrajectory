...
class Runner(object):
    ...
    def __init__(self, lang, threshold=config.THRESHOLD):
    ...
    def add(self, dir, type=util.CURRENT, name=None):
        ...
        with open(s.tmpfile(self.tmpdir), "w", errors="ignore") as tmpf:
            s.lines = 0
            for file in sorted(files):
                ...
                with open(file, "rU", errors="replace") as f:
                    lines = f.readlines()
    ...
    def run(self, npairs=config.NPAIRS):
        prevwd = os.getcwd()
