...
def plot_diffs(output_dir, year_q):
  ...
  #plot_consecline_stats(output_dir, year_q)
    ...
    for sim_type, time_type in axes_options:
      for use_mt, use_rank in graph_options:
        ...
        for uname in top_sims:
          ...
          for posix_time in top_sims[uname]:
            ...
            for line in top_sims[uname][posix_time]:
              name_other, token, percent_self, percent_other, _, _ = \
        ...
        for online_ind in [0,1]:
          ...
          lrs = all_np[:,lr_ind]/float(max_lr)

          if sim_type == 'token':
            sims = all_np[:,token_ind]
          else: # percent
            sims = all_np[:,self_ind]
          if time_type == 'commit':
          ...
          if use_grades:
            ...
            c = m.to_rgba(grades)
          if online_ind == 0:
            ax.scatter(times, sims, marker=marker, color=c, lw=0, alpha=0.3,
                      s=msize*lrs)
          else:
            ax.scatter(times, sims, marker=marker, color=c, s=msize*lrs)
...
def plot_consecline_stats(output_dir, year_q, use_mt=True, rank=True):
    ...
    temp_list = [(posix_times[i],
                  ...
                  grade) \
                      ...
                      for j in range(len(consec_stats[uname][posix_times[i]][0]))\
                        if consec_stats[uname][posix_times[i]][0]]
    ...
    temp_list = [(posix_times[i],
                  ...
                  grade) \
                      ...
                      for j in range(len(consec_stats[uname][posix_times[i]][1]))\
                        if consec_stats[uname][posix_times[i]][1]]
  ...
  #### histogram first.
  bin_steps=20
  bins = np.linspace(0,np.round(np.log10(np.amax(all_inserts_np[:,1]))),num=bin_steps)
  ...
  n, bins, patches = ax_all[i_ind].hist(all_inserts_np[:,1],bins=10,
                log=True,alpha=0.5,fill=True,histtype='bar')
  print insert_bin_edges, insert_hist
  ...
  n, bins, patches = ax_all[d_ind].hist(all_deletes_np[:,1],bins=10,
                log=True,alpha=0.5,fill=True,histtype='bar')
  ax_all[d_ind].set_title('deletions')
  ax_all[d_ind].set_xticks(bins)
  ...
  for time_type in ['posix', 'commit', 'norm']:
    insert_times = all_inserts_np[:,0] # default is posix
    delete_times = all_deletes_np[:,0]
    if time_type is 'commit':
      insert_times = all_inserts_np[:,2]
      delete_times = all_deletes_np[:,2]
    elif time_type is 'norm':
      insert_times = all_inserts_np[:,3]
      delete_times = all_deletes_np[:,3]

    num_plots = 2
    fig_x, fig_y = 8, 6
    fig, ax_all = plt.subplots(num_plots, 1, figsize=(fig_x, num_plots*fig_y))
    i_ind, d_ind = 0, 1
    ax_all[i_ind].scatter(insert_times, np.log10(all_inserts_np[:,1]),
                          c=all_inserts_np[:,4], lw=0, alpha=0.2)
    ax_all[i_ind].set_title('Insertions (paste or edits)')
    ax_all[d_ind].scatter(delete_times, np.log10(all_deletes_np[:,1]),
                          c=all_deletes_np[:,4], lw=0, alpha=0.2)
    ax_all[d_ind].set_title('deletions (deletes)')
    print "max insert?", np.amax(all_inserts_np[:,1])
    ...
    if time_type is 'posix':
      ...
      for ax_std in ax_all:
        ...
        ax_std.set_ylabel('Number of commits')
    elif time_type is 'norm':
      for ax_std in ax_all:
        ax_std.set_xlim(0, 1.0)
        ax_std.set_ylabel('Normalized % completion')
